...
#Configure the k8s-master helm-cli
    ...
    config.vm.define "jenkins-node" do |jenkins|
        ...
        jenkins.vm.provision "ansible" do |ansible|
            ansible.playbook = "jenkins-setup/jenkins-playbook.yml"
            ansible.verbose = "-vv"
            ansible.extra_vars = {
    ...
    config.vm.define "vault-node" do |vault|
        vault.vm.box = IMAGE_NAME
        vault.vm.network "private_network", ip: "192.168.50.6"
        vault.vm.hostname = "vault-node"
        vault.vm.provision "ansible" do |ansible|
           ansible.playbook = "vault-setup/vault-playbook.yml"
            ansible.verbose ="-vv"
            ansible.extra_vars = {
                node_ip: "192.168.50.6",
            }
    end
end
